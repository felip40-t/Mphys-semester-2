...
def calculate_parameters(data1, data2, num_events_1, num_events_2):
    ...
    def calc_alpha_2(data):
        return 0.25 * np.sqrt(5 / np.pi) * (np.mean(3 * np.square(data) - 1))

    def calc_gamma1010(data1, data2):
        return 0.25 * (3 / np.pi) * calc_exp_cos_mix(data1, data2)
    ...
    def calc_mix_uncertainty(data1, data2, exp_cos_1, exp_cos_2, sigma_cos_1, sigma_cos_2, num_events):
        ...
        print(f"Mixed expectation value first order: {calc_exp_cos_mix(data1, data2)}")
        print(f"exp vals: {exp_cos_1 * exp_cos_2}")
        return 0
    ...
    parameters = {
        ...
        'A0_2': calc_A0(exp_sqr_val_2), 'A4_2': 0.219 * calc_A4(exp_val_2)
    }
    ...
    uncertainties = {
        ...
        'A4_2_uncertainty': 0.219 * 4 * sigma_cos_2,
        'alpha1_1_uncertainty': 0.219 * 0.5 * np.sqrt(3 / np.pi) * sigma_cos_1,
        'alpha1_2_uncertainty': 0.219 * 0.5 * np.sqrt(3 / np.pi) * sigma_cos_2,
    ...
    parameters.update({
        ...
        'alpha1_2': 0.219 * calc_alpha_1(data2), 'alpha2_2': calc_alpha_2(data2),
        'gamma1010': 0.219 * 0.219 * calc_gamma1010(data1, data2), 'gamma2020': calc_gamma2020(data1, data2)
    })
...
for dataset in ZZ_files:
    ...
    output_file = os.path.join(ZZ_path, f"parameters_{dataset.split('_')[-1].split('.')[0]}.txt")
