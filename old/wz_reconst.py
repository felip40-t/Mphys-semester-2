...
process_dir = os.path.join(mg5_install_dir, "pp_WZ_SM")  # Updated process directory
...
particle_directories = {
    ...
    'z': os.path.join(process_dir, "Plots and data/z"),      # z boson (lowercase)
}
...
particle_arrays = {
    ...
    'z': np.array([]),   # For storing the reconstructed z boson
}
...
def extract_data():
    ...
    for particle_name, directory in particle_directories.items():
        if particle_name not in ['w+', 'z']:  # Only read data for mu+, mu-, e+, ve
...
def reconstruct_particles():
    """
    Reconstruct the four-momenta of the w+ and z bosons.
    - w+ = e+ + ve
    - z  = mu+ + mu-
    """
    particle_arrays['w+'] = particle_arrays['e+'] + particle_arrays['ve']
    particle_arrays['z'] = particle_arrays['mu+'] + particle_arrays['mu-']

def write_particle(particle_name):
    """
    Write the reconstructed four-momentum of the particle (w+ or z) to a file.
    """
...
def main():
    # Extract 4-momenta of individual particles (mu+, mu-, e+, ve)
    extract_data()

    # Reconstruct the four-momenta of w+ and z bosons
